...
openai.api_key = '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'
...
def procesar_lineas(text_lines, doc):
    for line in text_lines:
        ...
        if line.startswith(
            ...
            p = doc.add_paragraph()
            start = 0
            for match in sorted(itertools.chain(comillas_pattern.finditer(line), motivo_ordinal_pattern.finditer(line)),
...
root.geometry("1300x830")  # Establece el tamaño de la ventana a 1300x800
...
fecha_cal = DateEntry(left_scrollable_frame, date_pattern='dd/mm/yyyy')
...
inadmisibilidad_check = ttk.Checkbutton(left_scrollable_frame, text="Inadmisibilidad del recurso",
...
infraccion_check = ttk.Checkbutton(left_scrollable_frame, text="Infracción de las normas procesales",
...
analizar_revisiones_button = ttk.Button(left_scrollable_frame, text="Analizar revisiones fácticas", state='disabled',
                                        command=lambda: ejecutar_en_hilo(analizar_revisiones_facticas))
...
sustantivas_check = ttk.Checkbutton(left_scrollable_frame, text="Infracción de normas sustantivas",
...
# Botón para 'Analizar revisiones fácticas'
analizar_censuras_button = ttk.Button(left_scrollable_frame, text="Analizar censuras jurídicas", state='disabled',
                                      command=lambda: ejecutar_en_hilo(analizar_censuras_juridicas))
...
for opcion in opciones_costas:
    ttk.Radiobutton(costas_frame, text=opcion, value=opcion, variable=costas_var, command=update_text,
...
left_canvas.pack(side="left", fill="both", expand=True, padx=(10, 0),
                 pady=0)  # Agregar un padding de 10 píxeles a la izquierda
