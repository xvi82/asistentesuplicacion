...
#crear hilos
def cargar_pdf_thread():
    # Ejecuta cargar_pdf en un hilo separado
    threading.Thread(target=cargar_pdf).start()

def analyze_pdf_thread():
    # Ejecuta analyze_pdf en un hilo separado
    threading.Thread(target=analyze_pdf).start()

def manejar_pdf_thread():
    # Ejecuta analyze_pdf en un hilo separado
    threading.Thread(target=manejar_proceso).start()

def manejar2_pdf_thread():
    # Ejecuta analyze_pdf en un hilo separado
    threading.Thread(target=manejar_proceso2).start()
...
PARTIDO_JUZGADO_MAP = {
    ...
    "Las Palmas de Gran Canaria": [str(i) for i in range(1, 12)]
}
...
def extract_info_from_pdf(pdf_path):
    with open(pdf_path, 'rb') as pdf_file:
        ...
        # Crear el prompt para el resumen de la sentencia
        prompt_for_summary = (
            f"Eres un Juez experto en Derecho Laboral y de la Seguridad Social, actua como tal, tienes que hacer un resumen de 370 palabras del sentido de una sentencia y las razones para estimar o desestimar. Ten en cuenta solo el texto que se encuentra entre las secciones 'FUNDAMENTOS DE DERECHO' y 'FALLO', es la única parte importante, no tengas en cuenta nada más. Habla siempre en pasado. Obvia el apartado sobre los recursos. Utiliza a lo largo del texto expresiones como 'la sentencia de instancia apreció/consideró probado/resolvió/entendió'. Empieza el resumen con la expresión 'La sentencia de instancia estimaba/desestimaba la demanda'. La sentencia es la siguiente:\n: {full_text}"
        )

        # Obtener el resumen de GPT-4 (suponiendo que la función get_response2 está definida y funciona correctamente)
        summary_response = get_response2(prompt_for_summary)
        print(summary_response)  # Imprimir el resumen

        # Procesar la primera página para el primer prompt
        first_page_text = reader.pages[0].extract_text()
        # Crear el prompt para la extracción de datos de la primera página
        combined_prompt = (
        ...
        # Convertir números escritos en palabras a números y extraer datos
        response_parts = response.split(" ")
        for idx, part in enumerate(response_parts):
            response_parts[idx] = word_to_number(part)
        ...
        if extracted_data:
            ...
            # Actualizas las variables y los widgets correspondientes
            ...
            # Para asegurar que los widgets se actualizan, puedes llamar a sus funciones de actualización si existen
            update_text()

def activar_boton_derecho():
    resolver_button['state'] = 'normal'  # Activar el botón derecho


pdf_path = None  # Esto se define en la parte superior del script
def cargar_pdf():
    global pdf_path  # Indicar que vamos a usar la variable global
    progress_bar.grid(row=0, column=2, padx=10, pady=10, sticky="ew")  # Mostras la barra de progreso
...
def save_to_doc():
    doc = Document()
    # Configurar el estilo de fuente y párrafo por defecto
    style = doc.styles['Normal']
    style.font.name = 'Times New Roman'
    style.font.size = Pt(12)
    style.paragraph_format.space_before = Pt(0)  # Espacio anterior 0 puntos
    style.paragraph_format.space_after = Pt(0)   # Espacio posterior 0 puntos
    style.paragraph_format.line_spacing = 1.0  # interlineado simple
    style.paragraph_format.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY

    # Dividir el texto en líneas para aplicar estilos por separado
    ...
    for line in text_lines:
        if line.startswith(
            ...
            p = doc.add_paragraph()
            p.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
            parts = re.split(r'(".*?")', line)  # Separa el texto manteniendo el grupo entre comillas como un elemento separado
            # Recorre todas las partes de la línea y formatea según sea necesario
            for part in parts:
                if part.startswith('"') and part.endswith('"'):
                    run = p.add_run(part)  # Añadir comillas dobles y el texto en cursiva
                    run.italic = True
                else:
                    p.add_run(part)  # Resto del texto
        else:
            p = doc.add_paragraph(line)
            p.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY

    # Guardar el documento...
    # Utiliza este diccionario para traducir el nombre del mes al español
    meses = {
    ...
    # Luego, actualiza la línea que define current_time con lo siguiente:
    now = datetime.datetime.now()
    nombre_mes = meses[now.strftime("%B")]
    current_time = now.strftime(f"editado el día %d de {nombre_mes} de %Y a las %H-%M")
    # Asegúrate de que estas variables contengan los valores correctos.
    numero_autos = autos_var.get()
    año_autos = autos_year_var.get()
    # Obtiene la ruta del directorio donde se ejecuta el script
    directory_path = os.path.dirname(os.path.realpath(__file__))
    # Crea el nombre de archivo con la ruta completa
    filename = os.path.join(directory_path, f"Rec. {numero_autos}-{año_autos} {current_time}.docx")
    doc.save(filename)
    print(f"Documento guardado en: {filename}")  # Esto imprimirá la ruta del archivo guardado

    try:
        # En Windows, usa startfile()
        if os.name == 'nt':
...
def clear_all():
    # Resetear variables de texto y fecha
    for var in [partido_var, recurrente_var, autos_var, autos_year_var, sintesis_var,
                articulos_var, jurisprudencia_var, revocacion_var, revocacion_parcial_var,
                razon_var, juzgado_var, revisiones_var, censuras_var]:
        var.set('')

    # Resetear fecha a la actual
    fecha_cal.set_date(datetime.date.today())
    # Limpiar texto de la interfaz derecha
    right_text.delete("1.0", tk.END)

    # Desactivar todos los checkboxes y radiobuttons
    for chk_var in [cuestiones_var, inadmisibilidad_var, infraccion_var,
                    revision_var, sustantivas_var]:
        chk_var.set(False)

    # Desactivar radiobuttons de costas (ninguno seleccionado)
    costas_var.set(None)
    # Reestablecer opciones por defecto para los Radiobuttons de "Trabajador" o "Empresa"
    trabajador_empresa_var.set("Trabajador")

    # Desmarcar todos los checkboxes dentro del frame de tipos de infracción de normas procesales
    for tipo_var in tipos_vars:
        tipo_var.set(False)

    # Si se están manejando elementos con la función `update_juzgado` o similar,
    # deberían ser llamados para actualizar la interfaz
    update_juzgado()
    # Actualizar visibilidad de los frames si la visibilidad depende de estados de variables
    toggle_infraccion()
    toggle_sustantivas()
...
def update_text(*args):
    ...
    ordinals = ["SEGUNDO.-", "TERCERO.-", "CUARTO.-", "QUINTO.-", "SEXTO.-", "SEPTIMO.-", "OCTAVO.-"]
...
def interpret_response(response_str):
    ...
    if len(elements) == 5:
        ...
        if articulos_infringidos:
            ...
            toggle_sustantivas()  # Mostrar los campos adicionales
        else:
        ...
        # Unir las frases con comas y 'y' antes de la última parte
        frase_final = f"\nDisconforme la parte actuante {nombre_recurrente}, interpone el presente recurso de suplicación articulando " + ', '.join(
            partes_frase[:-1]) + f" y {partes_frase[-1]}" if len(partes_frase) > 1 else partes_frase[0]
        frase_final += ", pretendiendo la revocación de la Sentencia con la condena de la parte demandada.\n"
...
def manejar_proceso():
    pdf1 = pdf_path
    pdf2 = pdf_path_for_text_extraction
    texto_unido = unir_pdfs_y_anadir_texto(pdf1, pdf2)
    numero_de_repeticiones = motivos_b  # Puedes cambiar este número según lo necesites
    respuesta_final=""

    for i in range(numero_de_repeticiones):
        ordinal = ["primero", "segundo", "tercero", "cuarto", "quinto", "sexto", "séptimo", "octavo"]
        prompt_modificado = promptrevision.replace("primero", ordinal[i])

        respuesta = get_response4(prompt_modificado, texto_unido)
...
promptrevision = """Eres un experto analizador de recursos de suplicación. Quiero que analices el Recurso. Los recursos de suplicación pueden interponerse por el art. 193.a) LRJS, art. 193.b) LRJS o art. 193.c) LRJS.
            ...
            - Una vez que los encuentre centrate sólo en el primero. Sólo quiero que analices el primero. El motivo puede ser una modificación de un hecho probado o la adición de un nuevo hecho probado o de un nuevo párrafo a un hecho probado ya existente. 
...
"Como primero motivo de revisión fáctica, la parte recurrente, [nombre del recurrente, no confundir con el del abogado], interesa la modificación del Hecho Probado [decir qué número de hecho probado es], cuya redacción original es:

[ve a la SENTENCIA y copia exactamente el hecho probado que se pretende modificar]
...
"Como primero motivo de revisión fáctica, la parte recurrente, [nombre del recurrente, no confundir con el del abogado], interesa la adición de un nuevo Hecho Probado [decir qué número de hecho probado es]/de un nuevo párrafo al Hecho Probado [decir qué número de hecho probado es], cuya redacción sería la siguiente:
...
def manejar_proceso2():
    ...
    for i in range(numero_de_repeticiones):
        ordinal = ["primero", "segundo", "tercero", "cuarto", "quinto", "sexto", "séptimo", "octavo"]
        prompt_modificado = promptcensura.replace("primero", ordinal[i])
...
promptcensura = """Eres un experto analizador de recursos de suplicación. Quiero que analices el Recurso. Los recursos de suplicación pueden interponerse por el art. 193.a) LRJS, art. 193.b) LRJS o art. 193.c) LRJS.
            ...
            - Una vez que los encuentres centrate sólo en el primero. Sólo quiero que analices el primero.
...
"Como primero motivo censura jurídica, la parte recurrente, [nombre del recurrente, no confundir con el del abogado, poner en mayúscula sólo las primeras letras de cada palabra], interesa la revocación de la sentencia de instancia al considerar que se ha producido la infracción [decir los artículos que el recurrente dice que se han infringido en el primero motivo de la letra c)], a saber [aquí empieza el resumen]."
...
root.geometry("1300x720")  # Establece el tamaño de la ventana a 800x600
...
left_scrollable_frame.bind(
    ...
    lambda e: left_canvas.configure(
        ...
        width=e.width + 20  # Agregar un padding interno de 10 píxeles a cada lado
    )
)
...
cargar_button = ttk.Button(opciones_frame, text="Cargar Sentencia", command=cargar_pdf_thread)
cargar_button.grid(row=0, column=0, sticky="ew", padx=2, pady=2)

# Botón "resolver" dentro del frame
resolver_button = ttk.Button(opciones_frame, text="Cargar Recurso", state='disabled', command=analyze_pdf_thread)
resolver_button.grid(row=0, column=1, sticky="ew", padx=2, pady=2)




# Asegúrate de ajustar el número de fila (row) para que el checkbox y los botones se posicionen correctamente en tu GUI.
...
for partido in ["Las Palmas de Gran Canaria", "Arrecife", "Puerto del Rosario", "Galdar"]:
    ...
    check.pack(side="top", anchor="w", pady=2)
...
autos_label.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0,5))

autos_entry = ttk.Entry(autos_frame, textvariable=autos_var)
...
recurrente_label.grid(row=7, column=0, sticky="ew", padx=10, pady=(5,0))
recurrente_entry = ttk.Entry(left_scrollable_frame, textvariable=recurrente_var)
recurrente_entry.grid(row=8, column=0, sticky="ew", padx=10, pady=(0,5))
...
sintesis_label.grid(row=9, column=0, sticky="ew", padx=10, pady=(5,0))
sintesis_entry = ttk.Entry(left_scrollable_frame, textvariable=sintesis_var, width=40)
sintesis_entry.grid(row=10, column=0, sticky="ew", padx=10, pady=(0,5))
...
cuestiones_check.grid(row=11, column=0, sticky="w", padx=10, pady=2)
...
inadmisibilidad_check.grid(row=12, column=0, sticky="w", padx=10, pady=2)
...
infraccion_check.grid(row=13, column=0, sticky="w", padx=10, pady=2)
...
def toggle_infraccion():
    if infraccion_var.get():
        ...
        dynamic_frame.grid(row=14, column=0, sticky="ew", padx=20, pady=2)
    else:
...
revision_check.grid(row=15, column=0, sticky="w", padx=10, pady=2)

# Botón para 'Analizar revisiones fácticas'
analizar_revisiones_button = ttk.Button(left_scrollable_frame, text="Analizar revisiones fácticas", command=manejar_pdf_thread)
analizar_revisiones_button.grid(row=16, column=0, padx=10, pady=10, sticky="ew")
...
sustantivas_check.grid(row=17, column=0, sticky="w", padx=10, pady=2)

# Cuadros de texto para 'Infracción de normas sustantivas'
sustantivas_frame = ttk.Frame(left_scrollable_frame)
articulos_var = tk.StringVar()
jurisprudencia_var = tk.StringVar()

def toggle_sustantivas():
    if sustantivas_var.get():
        sustantivas_frame.grid(row=18, column=0, sticky="ew", padx=20, pady=2)
    else:
        sustantivas_frame.grid_forget()

sustantivas_var.trace("w", lambda *_: toggle_sustantivas())

# Artículos infringidos
articulos_label = ttk.Label(sustantivas_frame, text="Artículos infringidos")
articulos_label.grid(row=0, column=0, sticky="ew", pady=(0,0))
articulos_entry = ttk.Entry(sustantivas_frame, textvariable=articulos_var, width=40)
articulos_entry.grid(row=1, column=0, sticky="ew", pady=(0,0))

# Jurisprudencia infringida
jurisprudencia_label = ttk.Label(sustantivas_frame, text="Jurisprudencia infringida")
jurisprudencia_label.grid(row=2, column=0, sticky="ew", pady=(5,0))
jurisprudencia_entry = ttk.Entry(sustantivas_frame, textvariable=jurisprudencia_var, width=40)
jurisprudencia_entry.grid(row=3, column=0, sticky="ew", pady=(0,5))

# Botón para 'Analizar revisiones fácticas'
analizar_censuras_button = ttk.Button(left_scrollable_frame, text="Analizar censuras jurídicas", command=manejar2_pdf_thread)
analizar_censuras_button.grid(row=19, column=0, padx=10, pady=10, sticky="ew")
analizar_censuras_button.grid_remove()  # Ocultar el botón

def toggle_censuras_button():
    if sustantivas_var.get():
...
# Vincular la función toggle_revision_button al cambio de estado del checkbox 'Revisión de hechos probados'
sustantivas_var.trace("w", lambda *args: toggle_censuras_button())

# Llamar a la función una vez para configurar el estado inicial del botón
toggle_censuras_button()

# Frame "Costas, depósitos y consignaciones"
costas_frame = ttk.LabelFrame(left_scrollable_frame, text="Costas, depósitos y consignaciones", borderwidth=1, relief="solid")
costas_frame.grid(row=20, column=0, sticky="ew", padx=10, pady=5)
...
for option in textos_costas.keys():
    ttk.Radiobutton(costas_frame, text=option, value=option, variable=costas_var, command=update_text).pack(anchor="w", padx=10, pady=2)
...
estimacion_frame.grid(row=21, column=0, sticky="ew", padx=10, pady=5)
...
ttk.Label(estimacion_frame, text="Revocación").pack(anchor="w", pady=5)
revocacion_var = tk.StringVar()
revocacion_entry = ttk.Entry(estimacion_frame, textvariable=revocacion_var, width=40)
revocacion_entry.pack(anchor="w", pady=5, padx=5)
...
estimacion_parcial_frame.grid(row=22, column=0, sticky="ew", padx=10, pady=5)
...
ttk.Label(estimacion_parcial_frame, text="Revocación parcial").pack(anchor="w", pady=5)
...
cuantia_costas_entry.grid(row=24, column=0, sticky="ew", padx=10, pady=5)
...
razones_frame.grid(row=24, column=0, sticky="ew", padx=10, pady=5)
...
button_frame.grid(row=100, column=0, sticky="ew", pady=10)
...
progress_bar.grid(row=0, column=2, padx=10, pady=10, sticky="ew")
