...
def abrir_nueva_ventana(texto_seleccionado):
    ...
    def abrir_ventana_feedback():
        def enviar_feedback():
            ...
            regla = openai.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system",
        ...
        ventana_feedback.update()

        # Poner blanco el titlebar en Win11
        HWND = windll.user32.GetParent(ventana_feedback.winfo_id())
        title_bar_color = 0x00FFFFFF
        windll.dwmapi.DwmSetWindowAttribute(HWND, 35, byref(c_int(title_bar_color)), sizeof(c_int))

        cuadro_feedback = tk.Text(ventana_feedback, wrap=tk.WORD, width=45, height=10, font=("SegoeUI", 10),
                                  relief='groove', bd=2)
    ...
    left_text = tk.Text(frame, wrap=tk.WORD, width=45, height=26, font=("SegoeUI", 10), relief='groove', bd=2)
    ...
    right_text = tk.Text(frame, wrap=tk.WORD, width=45, height=26, font=("SegoeUI", 10), relief='groove', bd=2)
...
def extract_info_from_pdf(pdf_path):
    with open(pdf_path, 'rb') as pdf_file:
        ...
        # Construir una expresión regular para manejar ambas formas de "FALLO"
        fallo_regex = r"F\s*A\s*L\s*L\s*O"

        # Buscar el texto entre "DERECHO" y la expresión regular de "FALLO"
        match = re.search(r"DERECHO(.*?)" + fallo_regex, full_text, re.DOTALL)

        if match:
            extracted_text = match.group(1)
        else:
            extracted_text = "No se encontró el texto entre DERECHO y FALLO"

        prompt_for_summary = create_summary_prompt(extracted_text)
        summary_response = get_response(prompt_for_summary, "gpt-4o")
...
def create_data_extraction_prompt(first_page_text):
    return (
        ...
        "- Indica el número de procedimiento sin ceros al principio, tiene que dar el número y el año, con este formato numero/año, por ejemplo 123/2023.\n"
        ...
        "- Especifíca el nombre del partido judicial, puede ser: Las Palmas de Gran Canaria, Arrecife, Gáldar o Puerto del Rosario.\n"
...
def update_ui_components(extracted_data, summary_response):
    ...
    # Separar el número de procedimiento por el carácter '/'
    partes_numero_procedimiento = numero_procedimiento.split('/')
    if len(partes_numero_procedimiento) > 1:
        # Si hay al menos dos partes, se asume que la segunda es el año
        autos_year_var.set(partes_numero_procedimiento[1])
    else:
        # Si no, se puede dejar vacío o asignar un valor predeterminado
        autos_year_var.set("Año no disponible")
...
def cargar_pdf_sentencia():
    ...
    progress_bar.grid(row=1, column=0, columnspan=2, padx=2, pady=2, sticky="ew")  # Mostras la barra de progreso
...
motivo_ordinal_pattern = re.compile(
    r"(único|primer|segundo|tercer|cuarto|quinto|sexto|séptimo|octavo|noveno|décimo) motivo")
...
def save_to_doc():
    ...
    if not numero_recurso:
        ...
        recurso_entry.focus()
        left_scrollable_frame.move_view_to_start()
        return
...
def configurar_documento():
    ...
    # Cambiar las propiedades del core
    doc.core_properties.author = "Creado por Paco Pérez"
    doc.core_properties.title = "Título del Documento"  # Opcional, puedes establecer el título aquí
    doc.core_properties.description = "Descripción o comentarios del documento."  # Establecer comentarios

    # Puedes modificar otras propiedades aquí si es necesario
...
def procesar_lineas(text_lines, doc):
    for line in text_lines:
        ...
        if line.startswith(
            ...
            p.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
            start = 0
            matches = list(sorted(itertools.chain(comillas_pattern.finditer(line), motivo_ordinal_pattern.finditer(line)),
...
########### AQUÍ IRÍAN LAS FUNCIONES DE GUARDADO TEMPORAL

# Funciones para guardar y abrir borradores
def guardar_borrador():
    data = {
        'cuestiones_var': cuestiones_var.get(),
        'inadmisibilidad_var': inadmisibilidad_var.get(),
        'infraccion_var': infraccion_var.get(),
        'sintesis_var': sintesis_var.get(),
        'revision_var': revision_var.get(),
        'sustantivas_var': sustantivas_var.get(),
        'articulos_var': articulos_var.get(),
        'jurisprudencia_var': jurisprudencia_var.get(),
        'trabajador_empresa_var': trabajador_empresa_var.get(),
        'revocacion_var': revocacion_var.get(),
        'revocacion_parcial_var': revocacion_parcial_var.get(),
        'cuantia_costas_var': cuantia_costas_var.get(),
        'razon_var': razon_var.get(),
        'anular_var': anular_var.get(),
        'costas_var': costas_var.get(),
        'partido_var': partido_var.get(),
        'juzgado_var': juzgado_var.get(),
        'fecha_cal': fecha_cal.get_date().isoformat(),
        'autos_var': autos_var.get(),
        'autos_year_var': autos_year_var.get(),
        'recurso_var': recurso_var.get(),
        'recurso_year_var': recurso_year_var.get(),
        'recurrente_var': recurrente_var.get(),
        'revisiones_var': revisiones_var.get(),
        'censuras_var': censuras_var.get(),
        'impugnante_var': impugnante_var.get()
    }
    file_path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("JSON files", "*.json")])
    if file_path:
        with open(file_path, 'w') as file:
            json.dump(data, file)
        messagebox.showinfo("Guardar Borrador", "Borrador guardado exitosamente.")
...
def abrir_borrador():
    ...
    if file_path:
        with open(file_path, 'r') as file:
            data = json.load(file)
        cuestiones_var.set(data['cuestiones_var'])
        inadmisibilidad_var.set(data['inadmisibilidad_var'])
        infraccion_var.set(data['infraccion_var'])
        sintesis_var.set(data['sintesis_var'])
        revision_var.set(data['revision_var'])
        sustantivas_var.set(data['sustantivas_var'])
        articulos_var.set(data['articulos_var'])
        jurisprudencia_var.set(data['jurisprudencia_var'])
        trabajador_empresa_var.set(data['trabajador_empresa_var'])
        revocacion_var.set(data['revocacion_var'])
        revocacion_parcial_var.set(data['revocacion_parcial_var'])
        cuantia_costas_var.set(data['cuantia_costas_var'])
        razon_var.set(data['razon_var'])
        anular_var.set(data['anular_var'])
        costas_var.set(data['costas_var'])
        partido_var.set(data['partido_var'])
        juzgado_var.set(data['juzgado_var'])
        fecha_cal.set_date(datetime.date.fromisoformat(data['fecha_cal']))
        autos_var.set(data['autos_var'])
        autos_year_var.set(data['autos_year_var'])
        recurso_var.set(data['recurso_var'])
        recurso_year_var.set(data['recurso_year_var'])
        recurrente_var.set(data['recurrente_var'])
        revisiones_var.set(data['revisiones_var'])
        censuras_var.set(data['censuras_var'])
        impugnante_var.set(data['impugnante_var'])

        # Llama a update_text para actualizar el contenido de los cuadros de texto
        update_text(data)

        messagebox.showinfo("Abrir Borrador", "Borrador cargado exitosamente.")
...
""")
	...
	Dicha doctrina jurisprudencial, así mismo señala que no basta que el recurso cite la disposición legal conculcada si contiene diversos artículos, sino que es preciso que se señale el específico precepto que se entiende vulnerado, y si el precepto contiene varios apartados resulta igualmente indispensable señalar expresamente cuál de ellos se reputa infringido.""")
            ...
            content_sections.append(
                f"""	La parte recurrente interesa la revocación de la sentencia, al amparo del art. 193.c) LRJS, alegando la infracción de los arts. {articulos}.
	Con carácter previo debemos indicar – por lo que respecta a las normas citadas por la parte recurrente en su recurso como infringidas – que el recurso de suplicación no es una apelación o segunda instancia, sino un recurso extraordinario sujeto a motivos tasados en cuya formulación se han de respetar los requisitos legales.
	Los motivos basados en el apartado c) del art. 193 LRJS se destinan a la impugnación del fallo por error in iudicando, y el recurrente tiene la carga de:
		a)	Citar debidamente el precepto o preceptos sustantivos y en su caso la jurisprudencia que, a su juicio, han sido vulnerados por el fallo de la sentencia, articulando motivos separados para cada precepto o grupo de preceptos que guarden unidad temática.
		b)	Razonar la pertinencia y fundamentación de los motivos (art. 196.2 LRJS) lo cual exige argumentar la conexión entre el contenido normativo de las normas o jurisprudencia citadas y el litigio, mostrando cómo su correcta aplicación debería haber llevado a dar distinta solución al debate.
...
FALLAMOS
	...
	ANULAR la sentencia del Juzgado de lo Social nº {juzgado_var.get()} de los de {partido_var.get()}, de fecha {fecha_a_texto(fecha_cal.get_date())}, dictada en autos nº {autos_var.get()}/{autos_year_var.get()}, para que el Juzgador, con plena libertad de criterio, proceda a {anular_var.get()}.
    ...
    # Definir palabras o patrones para aplicar estilos
    palabras_en_negrita_inicio_linea = ['FALLAMOS', "PRIMERO.-", "SEGUNDO.-", "TERCERO.-", "CUARTO.-", "QUINTO.-", "SEXTO.-", "SÉPTIMO.-"]
    palabras_en_negrita_siempre = ['Cuestiones de orden público', "Infracción de normas sustantivas y de la jurisprudencia", "Costas, depósitos y consignaciones", "Revisión de hechos probados", "Infracción de normas procesales", "Inadmisibilidad del recurso", "Síntesis de la litis", 'único motivo', 'primer motivo', 'segundo motivo', 'tercer motivo', 'cuarto motivo', 'quinto motivo']
    lineas_centrales = ["FALLAMOS"]

    # Aplicar estilo de negrita a palabras al inicio de línea
    for palabra in palabras_en_negrita_inicio_linea:
        start_index = '1.0'
        while True:
            start_index = right_text.search(palabra, start_index, stopindex=tk.END)
            if not start_index:
                break
            # Verificar si la palabra está al principio de una línea
            if start_index.split('.')[1] == '0':
                end_index = f"{start_index}+{len(palabra)}c"
                right_text.tag_add("bold", start_index, end_index)
            start_index = f"{start_index}+{len(palabra)}c"

    # Aplicar estilo de negrita a palabras en cualquier posición
    for palabra in palabras_en_negrita_siempre:
        start_index = '1.0'
        while True:
            start_index = right_text.search(palabra, start_index, stopindex=tk.END)
            if not start_index:
    ...
    for linea in lineas_centrales:
        start_index = '1.0'
        while True:
            start_index = right_text.search(linea, start_index, stopindex=tk.END)
            if not start_index:
                break
            end_index = f"{start_index} lineend"
            right_text.tag_add("center", start_index, end_index)
            start_index = end_index

    # Este patrón encuentra texto entre comillas dobles o simples
    pattern = r'"([^"]*?)"|“(.*?)”|\'([^\']*?)\'|«(.*?)»'
    ...
    while True:
        ...
        # Calcular el índice de inicio y fin en el widget de texto
        start_index = right_text.search(match.group(), start_index, stopindex=tk.END)
        if not start_index:
            break
        end_index = f"{start_index}+{len(match.group())}c"

        right_text.tag_add("italic", start_index, end_index)
        start_index = end_index


def update_and_scroll(*args):
    # Primero actualiza el texto
    update_text()

    # Luego desplaza a la parte final del texto
    right_text.see("end")
...
def generar_respuestas(prompt_base, numero_de_repeticiones, pdf2):
    ...
    for i in range(numero_de_repeticiones):
        ...
        partes_prompt = prompt_modificado.split(
            "El motivo puede ser una modificación de un hecho probado o la adición de un nuevo hecho probado o de un nuevo párrafo a un hecho probado ya existente.")
        prompt_modificado = partes_prompt[
                                0] + frase_ordinales + "El motivo puede ser una modificación de un hecho probado o la adición de un nuevo hecho probado o de un nuevo párrafo a un hecho probado ya existente." + \
                            partes_prompt[1]
...
def abrir_ventana_modificacion(lugar):
    ...
    # Poner blanco el titlebar en Win11
    HWND = windll.user32.GetParent(modificacion.winfo_id())
    title_bar_color = 0x00FFFFFF
    windll.dwmapi.DwmSetWindowAttribute(HWND, 35, byref(c_int(title_bar_color)), sizeof(c_int))
    ...
    message_text = tk.Text(right_frame, font=("SegoeUI", 10), wrap='word', state='disabled', borderwidth=1,
                           relief='groove', bd=2)
...
def abrir_ventana_adicion(lugar):
    ...
    # Poner blanco el titlebar en Win11
    HWND = windll.user32.GetParent(adicion.winfo_id())
    title_bar_color = 0x00FFFFFF
    windll.dwmapi.DwmSetWindowAttribute(HWND, 35, byref(c_int(title_bar_color)), sizeof(c_int))
...
def mostrar_menu_derecha(event, lugar):
    ...
    opciones_censura = [("Desestima", "Por ende, se desestima este motivo de censura jurídica."),
                        ("Desestima y confirma",
                         "Por ende, se desestima este motivo de censura jurídica y se confirma la sentencia de instancia."),
                        "SEPARATOR",
	...
	En multitud de ocasiones el recurso a la utilización de los elementos que ofrece la cuantificación de las sanciones de la LISOS no resulta, por sí mismo, suficiente para cumplir con relativa precisión la doble función de resarcir el daño y de servir de elemento disuasorio para impedir futuras vulneraciones del derecho fundamental. Ello es debido a que la horquilla de la cuantificación de las sanciones en la LISOS para un mismo tipo de falta (leve, grave, muy grave) resulta ser excesivamente amplía. Por ello, el recurso a las sanciones de la LISOS debe ir acompañado de una valoración de las circunstancias concurrentes en el caso concreto. Aspectos tales como la antigüedad del trabajador en la empresa, la persistencia temporal de la vulneración del derecho fundamental, la intensidad del quebrantamiento del derecho, las consecuencias que se provoquen en la situación personal o social del trabajador o del sujeto titular del derecho infringido, la posible reincidencia en conductas vulneradoras, el carácter pluriofensivo de la lesión, el contexto en el que se haya podido producir la conducta o una actitud tendente a impedir la defensa y protección del derecho transgredido, entre otros que puedan valorarse atendidas las circunstancias de cada caso, deben constituir elementos a tener en cuenta en orden a la cuantificación de la indemnización."""),
                        ...
                        ("Límites al escrito de impugnación",
...
4. Cuando el documento o documentos aportados reúna todas las anteriores exigencias la Sala valorará en cada caso su alcance en la propia sentencia o auto que haya de dictar”"""),
                        ("Subsanación de defectos en el recurso",
...
Se trata de una consecuencia que no puede considerarse contraria al derecho a la tutela judicial efectiva. Con arreglo a reiterada jurisprudencia constitucional "La comprobación de los presupuestos procesales para la viabilidad de la acción puede volverse a abordar o reconsiderarse en la sentencia, de oficio o a instancia de parte, dando lugar, en su caso, a un pronunciamiento de inadmisión por falta de tales presupuestos" (Por todas, STC 200/2012, de 12 de noviembre).”"""),
                        ("Introducción de cuestiones nuevas",
...
"""),
    ...
    opciones_despues_separador_ordenadas = sorted(opciones_despues_separador,
                                                  key=lambda x: conteo_uso_opciones.get(x[1], 0), reverse=True)
    ...
    opciones_revision = [("Procede",
                         ...
                         ("Hecho predeterminante del fallo",
                         ...
                         ("Valoración prueba por el juez a quo",
                         ...
                         ("Valoración de la prueba en otro sentido",
...
class ScrollableFrame(customtkinter.CTkScrollableFrame):
    ...
    def move_view_to_start(self):
        self._parent_canvas.yview_moveto(0.0)
        self.update()

def disable_specific_widgets(container):
    for child in container.winfo_children():
        # Deshabilitar ttk.Entry
        if isinstance(child, ttk.Entry):
            child.configure(state='disabled')
        # Deshabilitar ttk.Radiobutton
        elif isinstance(child, ttk.Radiobutton):
            child.configure(state='disabled')
        # Deshabilitar ttk.Checkbutton
        elif isinstance(child, ttk.Checkbutton):
            child.configure(state='disabled')
        # Recursividad para widgets anidados
        else:
            disable_specific_widgets(child)


def enable_specific_widgets(container):
    for child in container.winfo_children():
        # Habilitar ttk.Entry
        if isinstance(child, ttk.Entry):
            child.configure(state='normal')
        # Habilitar ttk.Radiobutton
        elif isinstance(child, ttk.Radiobutton):
            child.configure(state='normal')
        # Habilitar ttk.Checkbutton
        elif isinstance(child, ttk.Checkbutton):
            child.configure(state='normal')
        # Habilitar ttk.Combobox
        elif isinstance(child, ttk.Combobox):
            child.configure(state='readonly')  # usualmente los Combobox están en modo 'readonly' en lugar de 'normal'
        # Recursividad para widgets anidados
        else:
            enable_specific_widgets(child)



root = tk.Tk()
root.title('Asistente a la Suplicación')
root.geometry("1300x760")  # Establece el tamaño de la ventana a 1300x760
root.bind("<Control-r>", lambda event: enable_specific_widgets(left_frame))
...
left_scrollable_frame = ScrollableFrame(left_frame, width=305, fg_color='white')
left_scrollable_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=False)
...
cargar_button = ttk.Button(opciones_frame, text="Cargar Sentencia",
                           command=lambda: ejecutar_en_hilo(cargar_pdf_sentencia), style='White.TButton')
cargar_button.grid(row=0, column=0, sticky="ew", padx=2, pady=2)

# Botón "resolver" dentro del frame
recurso_button = ttk.Button(opciones_frame, text="Cargar Recurso", state='disabled',
                            command=lambda: ejecutar_en_hilo(cargar_pdf_recurso), style='White.TButton')
...
for partido in ["Las Palmas de Gran Canaria", "Arrecife", "Puerto del Rosario", "Gáldar"]:
    check = ttk.Checkbutton(partido_frame, text=partido, variable=partido_var, onvalue=partido, offvalue='',
                            command=update_juzgado, style='White.TCheckbutton')
    check.pack(side="top", anchor="w", padx=5, pady=2)

vcmd = (root.register(solo_numeros), '%P')

# Labelframe para los checkboxes 'Fundamentos Jurídicos'
datos_frame = ttk.LabelFrame(left_scrollable_frame, text="Datos del recurso", style='White.TLabelframe')
datos_frame.grid(row=3, column=0, sticky="ew", padx=10, pady=(5,5))

# Juzgado de procedencia dropdown
juzgado_var = tk.StringVar()

# Crear un Frame para el label y el entry
juzgado_frame = ttk.Frame(datos_frame, style='White.TFrame')
juzgado_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=(5, 5))

# Label a la izquierda con minsize de 100
juzgado_label = ttk.Label(juzgado_frame, text="Juzgado", style='White.TLabel')
juzgado_label.grid(row=0, column=0, sticky="w", padx=(0, 10))
juzgado_frame.columnconfigure(0, minsize=120)  # Ajusta el minsize a 200 para igualar el tamaño

# Entry a la derecha que se expande en la columna 1
juzgado_options = ttk.Combobox(juzgado_frame, textvariable=juzgado_var)
juzgado_options.grid(row=0, column=1, sticky="ew")
juzgado_frame.columnconfigure(1, weight=1)  # Expande el Combobox en la columna 1

# Crear un Frame para el DateEntry
fecha_frame = ttk.Frame(datos_frame, style='White.TFrame')
fecha_frame.grid(row=5, column=0, sticky="ew", padx=10, pady=(5, 5))

# Label a la izquierda con minsize de 200 para igualar el tamaño con el otro Frame
fecha_label_left = ttk.Label(fecha_frame, text="Fecha sentencia", style='White.TLabel')
fecha_label_left.grid(row=0, column=0, sticky="w", padx=(0, 10))
fecha_frame.columnconfigure(0, minsize=120)

# DateEntry a la derecha que se expande en la columna 1
fecha_cal = DateEntry(fecha_frame, date_pattern='dd/mm/yyyy')
fecha_cal.grid(row=0, column=1, sticky="ew", padx=(0,0))
fecha_frame.columnconfigure(1, weight=1)  # Expande el DateEntry en la columna 1


# Autos de procedencia
autos_var = tk.StringVar()
autos_year_var = tk.StringVar()
autos_frame = ttk.Frame(datos_frame, style='White.TFrame')
autos_frame.grid(row=6, column=0, sticky="ew", padx=10, pady=5)

# Label a la izquierda con minsize de 100
autos_label = ttk.Label(autos_frame, text="Autos", style='White.TLabel')
autos_label.grid(row=0, column=0, sticky="w", padx=(0, 10))
autos_frame.columnconfigure(0, minsize=120)

# Entry en el centro que se expande en la columna 1
autos_entry = ttk.Entry(autos_frame, textvariable=autos_var, validate="key", validatecommand=vcmd, width=10)
autos_entry.grid(row=0, column=1, sticky="ew")

# Combobox a la derecha en la columna 2
autos_year_options = ttk.Combobox(autos_frame, textvariable=autos_year_var, values=['2024', '2023', '2022', '2021'], width=5)
autos_year_options.grid(row=0, column=2, sticky="e")

autos_frame.columnconfigure(1, minsize=20)  # Expande el Entry en la columna 1
autos_frame.columnconfigure(2, weight=1)


# Numero de recurso
recurso_var = tk.StringVar()
recurso_year_var = tk.StringVar()
recurso_frame = ttk.Frame(datos_frame, style='White.TFrame')
...
recurso_label.grid(row=0, column=0, sticky="w", padx=(0, 10))
recurso_frame.columnconfigure(0, minsize=120)

# Entry en el centro que se expande en la columna 1
recurso_entry = ttk.Entry(recurso_frame, textvariable=recurso_var, validate="key", validatecommand=vcmd, width=10)
recurso_entry.grid(row=0, column=1, sticky="ew")

# Combobox a la derecha en la columna 2
recurso_year_options = ttk.Combobox(recurso_frame, textvariable=recurso_year_var, values=['2024', '2023', '2022', '2021'], width=5)
recurso_year_options.grid(row=0, column=2, sticky="e", padx=(0,0))

recurso_frame.columnconfigure(1, minsize=20)
recurso_frame.columnconfigure(2, weight=1)
...
recurrente_frame = ttk.Frame(datos_frame, style='White.TFrame')
recurrente_frame.grid(row=8, column=0, sticky="ew", padx=10, pady=(5, 5))
recurrente_label = ttk.Label(recurrente_frame, text="Recurrente", style='White.TLabel')
recurrente_label.grid(row=0, column=0, sticky="w", padx=(0, 10))
recurrente_entry = ttk.Entry(recurrente_frame, textvariable=recurrente_var)
recurrente_entry.grid(row=0, column=1, sticky="ew")
recurrente_entry.bind("<Button-3>", lambda event: mostrar_menu(event, recurrente_entry))
recurrente_frame.columnconfigure(0, minsize=120)
recurrente_frame.columnconfigure(1, weight=1)

# Impugnante

impugnante_var = tk.StringVar()
impugnante_frame = ttk.Frame(datos_frame, style='White.TFrame')
impugnante_frame.grid(row=9, column=0, sticky="ew", padx=10, pady=(5, 5))
impugnante_label = ttk.Label(impugnante_frame, text="Impugnante", style='White.TLabel')
impugnante_label.grid(row=0, column=0, sticky="w", padx=(0, 10))
impugnante_entry = ttk.Entry(impugnante_frame, textvariable=impugnante_var)
impugnante_entry.grid(row=0, column=1, sticky="ew")
impugnante_entry.bind("<Button-3>", lambda event: mostrar_menu(event, impugnante_entry))
impugnante_frame.columnconfigure(0, minsize=120)
impugnante_frame.columnconfigure(1, weight=1)
...
def update_sintesis_with_impugnante(*args):
    ...
    # Chequear si la frase base ya está en el texto.
    base_presente = frase_base_impugnacion.strip() in sintesis_text

    if impugnante_name:
        if base_presente:
            # Actualizar solo el nombre del impugnante.
            start = sintesis_text.find(frase_base_impugnacion.strip()) + len(frase_base_impugnacion.strip())
            end = sintesis_text.find("\n", start) if "\n" in sintesis_text[start:] else len(sintesis_text)
            sintesis_text = sintesis_text[:start] + " " + impugnante_name + sintesis_text[end:] + "\n"
        else:
            # Añadir la nueva frase al final.
            sintesis_text += frase_base_impugnacion + " " + impugnante_name + ".\n"
    else:
        if base_presente:
            # Eliminar la frase relacionada con el impugnante.
            start = sintesis_text.find(frase_base_impugnacion.strip())
            end = sintesis_text.find("\n", start) if "\n" in sintesis_text[start:] else len(sintesis_text)
            sintesis_text = sintesis_text[:start].rstrip() + sintesis_text[end:]

    # Asegurar que el texto comience con una tabulación
    sintesis_text = "\t" + sintesis_text.lstrip()

    sintesis_var.set(sintesis_text)

# Retraso antes de actualizar (en milisegundos)
update_delay = 100

def delayed_update_sintesis(*args):
    # Función que retrasa la actualización real
    def delayed():
        update_sintesis_with_impugnante()
        root.after_cancel(update_sintesis_with_impugnante.id)
    # Cancelar cualquier actualización pendiente y planificar una nueva
    if hasattr(update_sintesis_with_impugnante, 'id'):
        root.after_cancel(update_sintesis_with_impugnante.id)
    update_sintesis_with_impugnante.id = root.after(update_delay, delayed)


# Asociar la nueva función al cambio de la variable impugnante_var
impugnante_var.trace("w", delayed_update_sintesis)
...
sintesis_frame = ttk.Frame(datos_frame, style='White.TFrame')
sintesis_frame.grid(row=12, column=0, sticky="ew", padx=10, pady=(5, 5))
sintesis_label = ttk.Label(sintesis_frame, text="Síntesis de la litis", style='White.TLabel')
sintesis_label.grid(row=0, column=0, sticky="ew", padx=(0,10))
sintesis_entry = ttk.Entry(sintesis_frame, textvariable=sintesis_var)
sintesis_entry.grid(row=0, column=1, sticky="ew")
sintesis_entry.bind("<Button-3>", lambda event: mostrar_menu(event, sintesis_entry))
sintesis_frame.columnconfigure(0, minsize=120)
sintesis_frame.columnconfigure(1, weight=1)
...
# Labelframe para los checkboxes 'Fundamentos Jurídicos'
fundamentos_frame = ttk.LabelFrame(left_scrollable_frame, text="Fundamentos Jurídicos", style='White.TLabelframe')
fundamentos_frame.grid(row=13, column=0, sticky="ew", padx=10, pady=(5,5))

# Checkbox 'Cuestiones de orden público'
cuestiones_var = tk.BooleanVar(value=False)
cuestiones_check = ttk.Checkbutton(fundamentos_frame, text="Cuestiones de orden público", variable=cuestiones_var,
...
inadmisibilidad_check = ttk.Checkbutton(fundamentos_frame, text="Inadmisibilidad del recurso",
                                        variable=inadmisibilidad_var, style='White.TCheckbutton')
...
dynamic_frame = ttk.Frame(fundamentos_frame, style='White.TFrame')

# Checkbox 'Infracción de las normas procesales'
infraccion_var = tk.BooleanVar(value=False)
infraccion_check = ttk.Checkbutton(fundamentos_frame, text="Infracción de las normas procesales",
                                   variable=infraccion_var, style='White.TCheckbutton')
...
revision_check = ttk.Checkbutton(fundamentos_frame, text="Revisión de hechos probados", variable=revision_var,
                                 style='White.TCheckbutton')
revision_check.grid(row=18, column=0, sticky="w", padx=8, pady=2)

# Botón para 'Analizar revisiones fácticas'
analizar_revisiones_button = ttk.Button(fundamentos_frame, text="Analizar revisiones fácticas", state='disabled',
                                        command=lambda: ejecutar_en_hilo(analizar_revisiones_facticas), style='White.TButton')
...
sustantivas_check = ttk.Checkbutton(fundamentos_frame, text="Infracción de normas sustantivas",
                                    variable=sustantivas_var, style='White.TCheckbutton')
...
articulos_label = ttk.Label(fundamentos_frame, text="Artículos infringidos", style='White.TLabel')
articulos_label.grid(row=22, column=0, sticky="ew", padx=10, pady=(5, 0))
articulos_entry = ttk.Entry(fundamentos_frame, textvariable=articulos_var, width=40)
...
jurisprudencia_label = ttk.Label(fundamentos_frame, text="Jurisprudencia infringida", style='White.TLabel')
jurisprudencia_label.grid(row=24, column=0, sticky="ew", padx=10, pady=(5, 0))
jurisprudencia_entry = ttk.Entry(fundamentos_frame, textvariable=jurisprudencia_var, width=40)
...
# Botón para 'Analizar revisiones fácticas'
analizar_censuras_button = ttk.Button(fundamentos_frame, text="Analizar censuras jurídicas", state='disabled',
                                      command=lambda: ejecutar_en_hilo(analizar_censuras_juridicas), style='White.TButton')
...
costas_frame = ttk.LabelFrame(left_scrollable_frame, text="Costas, depósitos y consignaciones", borderwidth=1,
                              relief="groove", style='White.TLabelframe')
...
for opcion in opciones_costas:
    ttk.Radiobutton(costas_frame, text=opcion, value=opcion, variable=costas_var,
                    command=lambda: update_and_scroll(),
                    style='White.TRadiobutton').pack(anchor="w", padx=5, pady=2)


# Subframe y widgets para "Estimación"
estimacion_frame = ttk.LabelFrame(left_scrollable_frame, text="Depósitos y Aseguramientos", borderwidth=2,
                                  relief="groove", style='White.TLabelframe')
...
ttk.Radiobutton(estimacion_frame, text="Trabajador", variable=trabajador_empresa_var, value="Trabajador",
                style='White.TRadiobutton').pack(anchor="w", pady=2)
ttk.Radiobutton(estimacion_frame, text="Empresa", variable=trabajador_empresa_var, value="Empresa",
                style='White.TRadiobutton').pack(anchor="w", pady=2)
...
estimacion_parcial_frame = ttk.LabelFrame(left_scrollable_frame, text="Depósitos y Aseguramientos", borderwidth=2,
                                          relief="groove", style='White.TLabelframe')
...
ttk.Radiobutton(estimacion_parcial_frame, text="Trabajador", variable=trabajador_empresa_var, value="Trabajador",
                style='White.TRadiobutton').pack(anchor="w", pady=2)
ttk.Radiobutton(estimacion_parcial_frame, text="Empresa", variable=trabajador_empresa_var, value="Empresa",
                style='White.TRadiobutton').pack(anchor="w", pady=2)
ttk.Label(estimacion_parcial_frame, text="Revocación parcial", style='White.TLabel').pack(anchor="w", padx=10,
                                                                                          pady=(5, 0))
...
razones_frame = ttk.LabelFrame(left_scrollable_frame, text="Razones para su no imposición", borderwidth=2,
                               relief="groove", style='White.TLabelframe')
...
# Widgets para "Anular"
anular_var = tk.StringVar()
anular_label = ttk.Label(left_scrollable_frame, text="Razones de anulación de la sentencia", style='White.TLabel')
anular_label.grid(row=30, column=0, sticky="ew", padx=10, pady=(5, 0))
anular_label.grid_remove()  # Ocultar por defecto
anular_entry = ttk.Entry(left_scrollable_frame, textvariable=anular_var, width=40)
anular_entry.grid(row=31, column=0, sticky="ew", padx=10, pady=(0, 5))
anular_entry.grid_remove()  # Ocultar por defecto
anular_entry.bind("<Button-3>", lambda event: mostrar_menu(event, anular_entry))
...
def toggle_costas_frames():
    ...
    frames_to_toggle = {
        ...
        "Anular": [anular_label, anular_entry]
    }
...
sustantivas_var.trace("w", update_and_scroll)
articulos_var.trace("w", update_and_scroll)
jurisprudencia_var.trace("w", update_and_scroll)
trabajador_empresa_var.trace("w", update_and_scroll)
revocacion_var.trace("w", update_and_scroll)
revocacion_parcial_var.trace("w", update_and_scroll)
cuantia_costas_var.trace("w", update_and_scroll)
razon_var.trace("w", update_and_scroll)
anular_var.trace("w", update_and_scroll)
costas_var.trace("w", update_and_scroll)
...
# Botón "Guardar"
save_button = ttk.Button(button_frame, text="Guardar", command=save_to_doc, style='White.TButton')
...
# Botón "Limpiar"
clear_button = ttk.Button(button_frame, text="Limpiar", command=lambda: ejecutar_en_hilo(clear_all), style='White.TButton')
...
progress_bar.grid(row=1, column=0, columnspan=2, padx=2, pady=2, sticky="ew")
progress_bar.grid_remove()  # Inicia con la barra de progreso oculta

# Segunda fila de botones
button_save_draft = ttk.Button(button_frame, text="Guardar Borrador", command=guardar_borrador, style='White.TButton')
button_save_draft.grid(row=2, column=0, padx=2, pady=2, sticky="ew")

button_open_draft = ttk.Button(button_frame, text="Abrir Borrador", command=abrir_borrador, style='White.TButton')
button_open_draft.grid(row=2, column=1, padx=2, pady=2, sticky="ew")
...
# Right text widget
right_text = tk.Text(right_frame, wrap="word", padx=10, pady=10, font=("SegoeUI", 11), relief='groove', bd=2)
...
right_text.bind("<Key>", lambda event: disable_specific_widgets(left_frame))

# Configurar etiquetas de estilo
right_text.tag_configure("bold", font=("SegoeUI", 11, "bold"))
right_text.tag_configure("italic", font=("SegoeUI", 11, "italic"))
right_text.tag_configure("center", justify="center")
